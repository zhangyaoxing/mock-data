...
## 2 How to Use
The tool utilizes [JSON schema](https://json-schema.org/), which is also used by MongoDB schema validator, to define data schema. Because JSON schema doesn't have the definition for values, we utlize the `description` field to define the values. The values can be generated by any of the [Faker](https://faker.readthedocs.io/en/stable/index.html) generator methods. To know which methods are available, checkout the [Standard Providers](https://faker.readthedocs.io/en/stable/providers.html) page, where you can find generator methods under each provider.
...
- `name`: Optional. Name of this schema.
- `count`: Optional. How many mock documents should be generated. Default to `1`.
- `output`: Optional. Output file name. Default to the same as schema file.
- `$jsonSchema`: Required. The JSON schema. [JSON schema](https://json-schema.org/) is a 3rd-party standard. You can find details in the website. In the [MongoDB document](https://www.mongodb.com/docs/manual/reference/operator/query/jsonSchema/) you can also find some examples.
...
The methods can be called in the format: `#<method>(<params>)#`. If you don't need any parameters, you can omit the whole `(<params>)`. The following are all valid value descriptions:
