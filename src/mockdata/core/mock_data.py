        ...
        for field_name, field_schema in properties.items():
            ...
            if bson_type not in ["object", "array"]:
                ...
                if description:
                    ...
                    if not gen_method:
                        self._logger.fatal("Generator method %s not found for field %s.", red(value_desc["gen_method"]), red(field_name))
                        sys.exit(1)
                    value = gen_method(*value_desc["params"])
                elif enum:
                    if not isinstance(enum, list) or len(enum) == 0:
                        self._logger.fatal("Enum for field %s must be a non-empty list.", red(field_name))
                        sys.exit(1)
                    value = self._fake.random_element(elements=enum)
                else:
                    self._logger.fatal("Field %s has no description or enum defined.", red(field_name))
                    sys.exit(1)
            
            # Convert value based on BSON type
            converted_value = self._convert_value(bson_type, value, field_name, field_schema, value_desc if 'value_desc' in locals() else None)
            obj[field_name] = converted_value
        
        return obj

    def _convert_value(self, bson_type: str, value, field_name: str, field_schema: dict, value_desc: dict = None):
